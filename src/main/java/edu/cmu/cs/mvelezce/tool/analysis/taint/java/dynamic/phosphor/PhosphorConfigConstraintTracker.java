...
public class PhosphorConfigConstraintTracker {
  ...
  private final Map<String, Map<ExecVarCtx, ExecConfigConstraints>> sinksToConfigConstraints =
      new HashMap<>();
  ...
  void deriveConstraints(
      Set<String> config, Map<String, Map<Set<String>, List<Set<String>>>> sinksToAnalysisTaints) {
  ...
  private void checkDiffExecVarCtx(
    ...
    for (Map.Entry<String, Map<Set<String>, List<Set<String>>>> analysisEntry :
        sinksToAnalysisTaints.entrySet()) {
  ...
  private void addConfigConstraints(
    ...
    for (Map.Entry<String, Map<Set<String>, List<Set<String>>>> analysisEntry :
      ...
      Map<ExecVarCtx, ExecConfigConstraints> execVarCtxsToConfigConstraints =
          sinksToConfigConstraints.get(analysisSink);
  ...
  private void addConfigConstraintsForSink(
    ...
    for (ExecVarCtx execVarCtx : execVarCtxs) {
      ...
      List<Set<ConfigConstraint>> existingExecConfigConstraints =
          execConfigConstraints.getConfigConstraints();
      addConfigConstraintsForCtx(execVarCtx, analysisTaints, existingExecConfigConstraints);
    }
  }

  private void addConfigConstraintsForCtx(
      ExecVarCtx execVarCtx,
    ...
    for (int i = 0; i < analysisTaints.size(); i++) {
      ...
      Set<ConfigConstraint> analysisTaintConfigConstraints =
          getTaintConfigConstraintsForCtx(execVarCtx, analysisTaint);
  ...
  private Set<ConfigConstraint> getTaintConfigConstraintsForCtx(
      ExecVarCtx execVarCtx, Set<String> taint) {
  ...
  private Set<ConfigConstraint> getConfigConstraints(
      Set<Set<String>> configs, Set<String> options) {
  ...
  private void addExecVarCtx(
      Map<String, Map<Set<String>, List<Set<String>>>> sinksToAnalysisTaints, Set<String> config) {
    for (Map.Entry<String, Map<Set<String>, List<Set<String>>>> analysisEntry :
        sinksToAnalysisTaints.entrySet()) {
      String analysisSink = analysisEntry.getKey();
      ...
      Map<Set<String>, List<Set<String>>> analysisCtxsToTaints = analysisEntry.getValue();
      addExecVarCtxsForSink(
          analysisCtxsToTaints.keySet(), existingExecVarCtxsToConfigConstraints, config);
    }
  }

  private void addExecVarCtxsForSink(
      Set<Set<String>> analysisCtxs,
      Map<ExecVarCtx, ExecConfigConstraints> existingExecVarCtxsToConfigConstraints,
      Set<String> config) {
  ...
  private void addInteractingExecVarCtx(
    ...
    //    Collection<ExecConfigConstraints> configConstraints =
    // existingExecVarCtxsToConfigConstraints
    //        .values();
    //
  ...
  Set<ConfigConstraint> getConfigConstraints() {
    ...
    for (Map<ExecVarCtx, ExecConfigConstraints> constraintsPerCtxAtSinks :
        sinksToConfigConstraints.values()) {
